...
const AccountManagementPage = () => {
  ...
  const fetchRoles = async () => {
    try {
      ...
      const teacherRole = roleResponse.data?.find(
        (role) => role.roleName === "Teacher"
      );
      const studentRole = roleResponse.data?.find(
  ...
  const fetchAccounts = async () => {
    ...
    try {
      ...
      if (user?.role === ROLE_NAME.SCHOOL_MANAGER) {
        ...
        if (!schoolAccountRelations?.data?.length) {
          setAccounts([]);
          return;
        }
        ...
        fetchedAccounts = accountResults
          .map((res) => res?.data?.[0])
          .filter(Boolean);
      } else if (user?.role === ROLE_NAME.ADMIN) {
        const accountResponse = await accountServices.get();
        fetchedAccounts = accountResponse.data || [];
      }

      fetchedAccounts.sort(
  ...
  const handleCreateAccount = async (accountData) => {
    try {
      ...
      if (accountResult.success || accountResult.data) {
        ...
        if (user?.schoolId) {
          const relationType =
            accountData.roleId === roleIds.student
              ? ROLE_NAME.STUDENT
              : ROLE_NAME.TEACHER;
  ...
  const handleImportAccounts = async (formData) => {
    try {
      ...
      if (response.data) {
        toast.success(
          `Tạo tài khoản hàng loạt thành công! Đã tạo ${
            response.data.createdAccounts?.length || 0
          } tài khoản`
        );
        ...
        if (response.data.errors && response.data.errors.length > 0) {
          ...
          toast.warning(
            `Có ${response.data.errors.length} lỗi trong quá trình import`
          );
        }
      ...
      if (error.response?.status === 400) {
        ...
        toast.error(
          "Tạo tài khoản hàng loạt thất bại. Vui lòng kiểm tra lại dữ liệu"
        );
      }
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">{roleDescription}</p>
        </div>
