...
const ProfilePage = () => {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      if (accountData.role === ROLE_NAME.SCHOOL_MANAGER && accountData.schoolId) {
        const schoolResponse = await schoolServices.get({ id: accountData.schoolId });
        additionalData.schoolInfo = schoolResponse.data[0];
        accountData.avatarUrl = schoolResponse.data[0]?.logoUrl || accountData.avatarUrl;
      }
      else if (accountData.role === ROLE_NAME.SUPPLIER && accountData.supplierId) {
        const supplierResponse = await supplierServices.get({ supplierId: accountData.supplierId });
        additionalData.supplierInfo = supplierResponse.data[0];
      }
      else if (accountData.role === ROLE_NAME.STUDENT || accountData.role === ROLE_NAME.TEACHER) {
        const relationResponse = await schoolAccountRelationServices.get({ accountId: targetUserId });
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            {isOwnProfile ? 'Thông tin cá nhân' : `Hồ sơ của ${profileData.fullName}`}
          </h1>
          ...
          <Card>
            <div className="border-b border-gray-200 mb-10">
              <nav className="flex space-x-8 px-6">
                {tabs.map((tab) => {
                  if (!isOwnProfile && (tab.id === 'wallet' || tab.id === 'security')) {
                    return null;
                  }
            ...
            <Card.Content>
              ...
              {activeTab === 'wallet' && isOwnProfile && <WalletTab user={targetUser} />}
