...
const AdminBoothsTab = ({ festival }) => {
  ...
  const loadBooths = async () => {
    try {
      setLoading(true);
      const response = await boothServices.get({
      ...
      const groupIds = [
        ...new Set(boothsData.map((booth) => booth.groupId).filter(Boolean)),
      ];
      const groupsData = {};
  ...
  const getBoothStats = () => {
    ...
    const approved = booths.filter(
      (booth) => booth.status === "approved"
    ).length;
    const rejected = booths.filter(
  ...
  const filteredBooths = booths
    .filter((booth) => {
      ...
      const matchesType =
        typeFilter === "all" || booth.boothType === typeFilter;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="mb-4 text-sm text-gray-600">
          Hiển thị <strong>{filteredBooths.length}</strong> trong tổng số{" "}
          <strong>{booths.length}</strong> gian hàng
        </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-green-600">
                    {stats.total > 0
                      ? Math.round(
                          ((stats.approved + stats.active) / stats.total) * 100
                        )
                      : 0}
                    %
                  </span>
