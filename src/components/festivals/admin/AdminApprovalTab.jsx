...
const AdminApprovalTab = ({ festival, festivalSchool, onStatusUpdate }) => {
    ...
    const handleApprove = async () => {
        ...
        try {
            setLoading(true);
            await festivalSchoolServices.updateApproveStatus({ id: festivalSchool.festivalSchoolId });
    ...
    if (!festivalSchool) {
        return (
            <div className="text-center py-8">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">Không tìm thấy thông tin đăng ký</h3>
                <p className="text-gray-600">Lễ hội này chưa có thông tin đăng ký từ trường.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div className={`p-6 rounded-lg border-2 ${getStatusColor(festivalSchool.status)}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="text-sm font-medium text-gray-700">Ngày đăng ký:</span>
                                <span className="text-sm text-gray-900">
                                    {convertToVietnamTimeWithFormat(festivalSchool.registrationDate)}
                                </span>
                            </div>
                            ...
                            {festivalSchool.approvalDate && (
                                <div className="flex items-center space-x-2">
                                    ...
                                    <span className="text-sm text-gray-900">
                                        {convertToVietnamTimeWithFormat(festivalSchool.approvalDate)}
                                    </span>
                                </div>
                            )}
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="text-sm font-medium text-gray-700">Festival ID:</span>
                                <span className="text-sm text-gray-900 font-mono">{festivalSchool.festivalId}</span>
                            </div>

                            <div className="flex items-center space-x-2">
                                <User size={16} className="text-gray-400" />
                                <span className="text-sm font-medium text-gray-700">School ID:</span>
                                <span className="text-sm text-gray-900 font-mono">{festivalSchool.schoolId}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            <div>
                                <span className="text-sm font-medium text-gray-700">Tên lễ hội:</span>
                                <p className="text-sm text-gray-900 mt-1">{festival.festivalName}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-900 mt-1">{festival.location}</p>
                            </div>
                ...
                {festivalSchool.rejectReason && (
                    <div className="mt-6 p-4 bg-red-100 rounded-lg border border-red-200">
                        ...
                        <p className="text-sm text-red-700">{festivalSchool.rejectReason}</p>
                    </div>
                )}
            </div>
            ...
            {festivalSchool.status === 'pending' && (
                <div className="bg-white p-6 rounded-lg border border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Thao tác kiểm duyệt</h3>
                    
                    <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        <h4 className="text-sm font-medium text-blue-800 mb-2">Hướng dẫn kiểm duyệt</h4>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="font-medium text-gray-900">
                                        {festivalSchool.status === 'approved' ? 'Duyệt lễ hội' : 'Từ chối lễ hội'}
                                    </p>
                                    <p className="text-sm text-gray-600">
                                        {convertToVietnamTimeWithFormat(festivalSchool.approvalDate || festivalSchool.registrationDate)}
                                    </p>
                                </div>
                            </div>
                            {getStatusBadge(festivalSchool.status, 'approval')}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            ...
                            <div>
                                ...
                                <p className="text-gray-900 mt-1">
                                    {festivalSchool.approvalDate ? (
                                        Math.ceil((new Date(festivalSchool.approvalDate) - new Date(festivalSchool.registrationDate)) / (1000 * 60 * 60 * 24))
                                    ) : 0} ngày
                                </p>
                            </div>
                        </div>
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h4 className="font-medium text-gray-900 mb-3">Cấu hình gian hàng</h4>
                        <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                                <span className="text-gray-600">Gian hàng đồ ăn:</span>
                                <span className="font-medium">{festival.maxFoodBooths || 0}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600">Gian hàng đồ uống:</span>
                                <span className="font-medium">{festival.maxBeverageBooths || 0}</span>
                            </div>
                            <div className="flex justify-between border-t pt-2 mt-2">
                                <span className="text-gray-600">Tổng cộng:</span>
                                <span className="font-medium">{(festival.maxFoodBooths || 0) + (festival.maxBeverageBooths || 0)}</span>
                            </div>
                        </div>
                    </div>

                    <div>
                        <h4 className="font-medium text-gray-900 mb-3">Thời gian đăng ký</h4>
                        <div className="space-y-2 text-sm">
                            <div>
                    ...
                    <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                        ...
                        <div className="mb-4">
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                                Lý do này sẽ được gửi đến ban tổ chức để họ có thể chỉnh sửa và đăng ký lại.
                            </p>
                        </div>
