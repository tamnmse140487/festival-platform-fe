...
const TransactionList = ({ transactions }) => {
  ...
  const getAmountDisplay = (transaction) => {
    ...
    if ([HISTORY_TYPE.PAYMENT, HISTORY_TYPE.TRANSFER].includes(type)) {
  ...
  const getStatusBadge = (type) => {
    ...
    const colorClasses = {
      ...
      [HISTORY_TYPE.REFUND]: 'bg-blue-50 text-blue-700 border-blue-200',
      [HISTORY_TYPE.CREATE_SUB_WALLET]: 'bg-purple-50 text-purple-700 border-purple-200',
      [HISTORY_TYPE.TRANSFER]: 'bg-orange-50 text-orange-700 border-orange-200',
      [HISTORY_TYPE.RETURN_TRANSFER]: 'bg-indigo-50 text-indigo-700 border-indigo-200',
      [HISTORY_TYPE.PAYMENT]: 'bg-red-50 text-red-700 border-red-200',
      [HISTORY_TYPE.COMMISSION]: 'bg-emerald-50 text-emerald-700 border-emerald-200',
    };

    return (
      <span className={`px-2 py-1 text-xs font-medium rounded-full border ${colorClasses[type] || 'bg-gray-50 text-gray-700 border-gray-200'}`}>
        {typeLabel}
      </span>
    );
  };
    ...
    <div>
      ...
      <div className="space-y-3">
        {sortedTransactions.map((transaction) => (
          <div key={transaction.historyId} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-lg hover:shadow-sm transition-shadow">
            <div className="flex items-center space-x-4">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center ${getBackgroundColor(transaction.type)}`}>
