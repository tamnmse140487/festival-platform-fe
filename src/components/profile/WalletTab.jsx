...
import { HISTORY_TYPE, REQUEST_TYPE, REQUEST_STATUS } from "../../utils/constants";
...
const WalletTab = ({ user }) => {
  ...
  const handleCreateWithdrawRequest = async () => {
    if (!user?.id || !walletData.walletId) {
      toast.error("Thiếu thông tin ví.");
      return;
    }
    ...
    try {
      await requestServices.create({
        ...
        data: JSON.stringify({ walletId: walletData.walletId, balance: walletData.balance }),
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        footer={[
          <Button key="cancel" variant="outline" onClick={() => setWithdrawVisible(false)}>
            Hủy
          </Button>,
          <Button key="ok" className="ml-3" onClick={handleCreateWithdrawRequest} disabled={isRequestingWithdraw}>
            {isRequestingWithdraw ? "Đang gửi..." : "Xác nhận tạo yêu cầu"}
          </Button>,
        ]}
      >
