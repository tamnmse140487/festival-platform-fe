...
const BoothRegistrationModal = ({
  ...
  const handleFormSubmit = (values) => {
    ...
    const invalidQuantityItems = selectedMenuItems.filter(
      (id) => !menuItemQuantities[id] || menuItemQuantities[id] <= 0
    );
    ...
    const invalidPriceItems = selectedMenuItems.filter(
      (id) => !menuItemPrices[id] || menuItemPrices[id] <= 0
    );
    ...
    if (invalidPriceRangeItems.length > 0) {
      const invalidItems = invalidPriceRangeItems
        .map((id) => {
          const item = menuItems.find((m) => m.itemId === id);
          return `${item.itemName} (${item.minPrice.toLocaleString(
            "vi-VN"
    ...
    const location = mapLocations.find(
      (loc) => loc.locationId === values.locationId
    );
  ...
  const handleConfirmRegistration = async () => {
    try {
      ...
      const imageUploadPromises = boothMenuItemResponses.map(
        async (response, index) => {
          ...
          if (imageFileList && imageFileList.length > 0) {
            ...
            try {
              await uploadService.uploadBoothMenuItemImage(
                file,
                boothMenuItemId
              );
            } catch (uploadError) {
              console.error(
                `Error uploading image for item ${menuItemId}:`,
                uploadError
              );
              toast.warning(
                `Không thể upload ảnh cho món ${
                  menuItems.find((m) => m.itemId === menuItemId)?.itemName
                }`
              );
            }
          }
      ...
      try {
        const response = await groupMemberServices.get({
          groupId: formData.groupId,
        });
        const teacherId = Array.isArray(response.data)
          ? response.data.find((s) => String(s.role) === "homeroom_teacher")
              ?.accountId
          : undefined;

        await notificationServices.createByType(
          NOTIFICATION_EVENT.BOOTH_PENDING,
          {
            data: {
              groupId: formData.groupId,
      ...
      toast.success(
        "Đăng ký gian hàng thành công! Vui lòng chờ giáo viên duyệt"
      );
      form.resetFields();
      ...
      >
        ...
        >
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: "Vui lòng chọn loại gian hàng" },
            ]}
          >
          ...
          >
            <TextArea rows={4} placeholder="Mô tả về gian hàng của bạn" />
          </Form.Item>

          {selectedBoothType && (
            <Form.Item label="Chọn món ăn/đồ uống" required>
              <div className="space-y-3 max-h-96 overflow-y-auto border border-gray-200 rounded-lg p-3">
                {filteredMenuItems.length > 0 ? (
                        ...
                        {isSelected && (
                          <div className="ml-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                              ...
                              <Input
                                placeholder={`${item.minPrice?.toLocaleString(
                                  "vi-VN"
                                )} - ${item.maxPrice?.toLocaleString("vi-VN")}`}
                            ...
                            <div>
                              ...
                              <Upload
                                ...
                                onChange={({ fileList }) =>
                                  handleImageUpload(item.itemId, fileList)
                                }
                                maxCount={1}
                                listType="picture-card"
                                className="menu-item-upload"
                              >
                                {(!menuItemImages[item.itemId] ||
                                  menuItemImages[item.itemId].length === 0) && (
                  ...
                  <p className="text-gray-500 text-center py-4">
                    Không có món {selectedBoothType === "food" ? "ăn" : "uống"}{" "}
                    nào trong thực đơn
                  </p>
                )}
              </div>
      ...
      >
        <div className="space-y-3">
          <p className="text-gray-700">
            Bạn có chắc muốn đăng ký gian hàng{" "}
            <strong>"{formData?.boothName}"</strong> tại vị trí{" "}
            <strong>
          ...
          <div className="bg-gray-50 p-3 rounded-lg text-sm space-y-2">
            <div>
              <strong>Tên gian hàng:</strong> {formData?.boothName}
            </div>
            <div>
              <strong>Loại:</strong>{" "}
              {formData?.boothType === "food" ? "Đồ ăn" : "Đồ uống"}
            </div>
            <div>
            ...
            <div>
              <strong>Số món đã chọn:</strong> {selectedMenuItems.length}
            </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-gray-600">
                          SL: {menuItemQuantities[itemId]} | Giá:{" "}
                          {formatPrice(menuItemPrices[itemId])} ₫
                        </span>
                      </li>
                    );
                  })}
